...
wizard-new-controller: .onyx
	...
	@mkdir -p ${CONTROLLERS}
	@cp -rf wizards/controller/* ${CONTROLLERS}
	@mv ${CONTROLLERS}/__ONYX_ControllerName ${TARGET_DIR}
	@find ${TARGET_DIR} -type f -exec sed -i 's/__KENAO_BoundedContext_LC/$(call lc,${BOUNDED_CONTEXT})/g' {} \;
	@find ${TARGET_DIR} -type f -exec sed -i 's/__KENAO_BackOrFront_LC/$(call lc,${BACK_OR_FRONT})/g' {} \;
	@find ${TARGET_DIR} -type f -exec sed -i 's/__ONYX_ControllerName_LC/$(call lc,${CONTROLLER_NAME})/g' {} \;
	@find ${TARGET_DIR} -type f -exec sed -i 's/__KENAO_BoundedContext/${BOUNDED_CONTEXT}/g' {} \;
	@find ${TARGET_DIR} -type f -exec sed -i 's/__KENAO_BackOrFront/${BACK_OR_FRONT}/g' {} \;
...
wizard-new-query: .onyx
	...
	@mkdir -p ${TARGET_DIR}
	# Copy files
	@cp -rf wizards/query/* ${TARGET_DIR}
	# Rename files
...
wizard-new-command: .onyx
	...
	@mkdir -p ${TARGET_DIR}
	# Copy files
	@cp -rf wizards/command/* ${TARGET_DIR}
	# Rename files
...
.PHONY: wizard-tree-bounded-context wizard-tree-aggregate-root 
